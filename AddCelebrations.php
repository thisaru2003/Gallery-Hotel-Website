...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($uploadOk == 0) {
        ...
        if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
            ...
            $stmt = $conn->prepare("INSERT INTO celebrations (title, description, image_url) VALUES (?, ?, ?)");
            $stmt->bind_param("sss", $title, $description, $image_url);

            if ($stmt->execute()) {
                $success_message = "New celebration added successfully!";
            } else {
...
<head>
    ...
    <title>Admin - Add Celebrations</title>
...
<body>
    ...
    <main>
        ...
        <form action="AddCelebrations.php" method="post" enctype="multipart/form-data">
            <div>
