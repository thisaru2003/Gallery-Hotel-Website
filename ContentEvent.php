...
<head>
    ...
    <title>The Gallery Hotel - Events</title>
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        echo '      <p>' . nl2br(htmlspecialchars($row["description"])) . '<br>';
        echo '      Event Date: ' . htmlspecialchars($row["event_date"]) . '<br>';
        echo '      Contact: ' . htmlspecialchars($row["contact_info"]) . '</p>';
        echo '    </div>';
        echo '  </div>';
        echo '</section>';
    }
} else {
    echo '<p style="text-align:center; margin-top:20px;">No events found.</p>';
}
